 ...
 */
export function processFile3(workbook, unifiedKootajSet) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const letter_date = normalizeLetterDate(letterDateRaw) || normalizeLetterDate(letterDateFallbackRaw);
    ...
    let match_status = 'EXTRACTION_FAILED';
  ...
  for (const [kootaj, group] of byKootaj) {
    ...
    const distinctLetters = [
      ...new Set(withLetter.map((r) => r.letter_number).filter(Boolean)),
    ];
  ...
  const matchedKootajs = new Set(
    matchedRows.map((r) => r.normalized_kootaj).filter(Boolean),
  );
  const unmatchedKootajs = new Set(
    unmatchedRows.map((r) => r.normalized_kootaj).filter(Boolean),
  );
