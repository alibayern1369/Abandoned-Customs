 ...
 */
export function buildKootajLevel(rows, fieldNames) {
  const level = {};
  const conflicts = [];
 ...
 */
export function buildItemDetails(rows, itemFields, extraFields = []) {
  return rows.map((row, idx) => {
    const item = { source_row_index: idx };
 ...
 */
export function computeSafeAggregates(rows, { weightFields = [], packageFields = [], valueFields = [] }) {
  const result = {};

  for (const f of weightFields) {
    const nums = rows.map((r) => normalizeNumber(r[f])).filter((n) => n != null);
    result[f] = {
