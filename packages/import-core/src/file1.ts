...
import type { ExcelRow, File1Result, KootajRecord, Workbook } from './types.js';
 ...
 */
export function processFile1(workbook: Workbook): File1Result {
  ...
  const kootajs: KootajRecord[] = [];
  let multiRow = 0;
  let multiWarehouse = 0;

  for (const g of groups.values()) {
    const { level, conflicts } = buildKootajLevel(g.rows, FILE1_FIELDS.kootajLevel);
    const items = buildItemDetails(g.rows, FILE1_FIELDS.itemLevel, [FILE1_FIELDS.warehouse, 'ردیف']);
