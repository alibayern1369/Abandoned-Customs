...
import type { ExcelRow, FieldValue, File2Result, KootajRecord, Workbook } from './types.js';

/**
 * Process File 2 against File 1 Kootaj set.
 * EXISTING → SKIP (no overwrite). NEW → collect parent + item details.
 */
export function processFile2(
  workbook: Workbook,
  ...
  const groups = new Map<
    string,
    { normalized_kootaj: string; original_values: Set<string>; rows: ExcelRow[] }
  >();
  let emptyKeyRows = 0;
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (original_value != null) g.original_values.add(original_value);
    g.rows.push({ ...row, _source_row: i + 2 });
  }

  const existing: KootajRecord[] = [];
  const neu: KootajRecord[] = [];
  ...
  existing.sort((a, b) =>
    a.normalized_kootaj.localeCompare(b.normalized_kootaj, 'en', { numeric: true }),
  );
  neu.sort((a, b) =>
    a.normalized_kootaj.localeCompare(b.normalized_kootaj, 'en', { numeric: true }),
  );
...
function flattenLevel(level: Record<string, FieldValue>): Record<string, string> {
  const out: Record<string, string> = {};
  for (const [k, v] of Object.entries(level)) {
    out[k] = v.value;
    if (v.rule === 'CONFLICT_KEEP_FIRST' && v.distinct_values) {
